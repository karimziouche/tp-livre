<?php

session_start();

if (!empty($_POST["book"])) {
    $_SESSION["cart"][] = $_POST["book"];
}
?>

<!DOCTYPE html>
<html lang="fr">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Ma bibliothèque</title>
    <link rel="stylesheet" href="style.css">
</head>
<body>
    <header class="header">
        <input type="checkbox" id="menu-toggle">
        <label for="menu-toggle" class="burger" aria-label="Ouvrir le menu">
            <span></span>
            <span></span>
            <span></span>
        </label>
        <nav class="menu">
            <?php
                if (!empty($_SESSION["name"])) {
                echo "Bienvenu à " . $_SESSION["name"];
                } else {
                echo "Bienvenue à Donkey";
                }
            ?>
            <a href="index.php">Accueil</a>
            <a href="cart.php">Mon panier</a>
            <a href="nous-contacter.php">Nous contacter</a>
            <a href="logout.php">Déconnexion</a>
        </nav>
        <h1>Ma bibliothèque</h1>
    </header>
    <main>
        <section class="introduction">
            <h2>Découvrez notre sélection</h2>
            <p>Retrouvez une sélection de grands classiques de la littérature. Cliquez sur le bouton détail pour découvrir chaque ouvrage.</p>
        </section>
        <section class="livres">
            <article class="carte-livre">
                <img src="lepetitprince.jpg" alt="Le petit prince">
                <div class="contenu-carte">
                    <h2>Le petit prince</h2>
                    <p>Antoine de Saint-exupéry</p>
                    <p>Publié en 1943</p>
                    <a href="détail-le-petit-prince.php" class="bouton">Détail</a>
                    <form method="post">
                        <button type="submit" 
                                name="book"
                                value="le-petit-prince"
                                class="bouton">
                            Ajouter au panier
                        </button>
                    </form>
                </div>
            </article>
            <article class="carte-livre">
                <img src="harrypotter.jpg" alt="Harry Potter">
                <div class="contenu-carte">
                    <h2>Harry Potter</h2>
                    <p>J.K Rowling</p>
                    <p>Publié en 1997</p>
                    <a href="détail-harry-potter.php" class="bouton">Détail</a>
                    <form method="post">
                        <button type="submit" 
                                name="book"
                                value="harry-potter"
                                class="bouton">
                            Ajouter au panier
                        </button>
                    </form>
                </div>
            </article>
            <article class="carte-livre">
                <img src="robindesbois.jpg" alt="Robin des bois">
                <div class="contenu-carte">
                    <h2>Robin des bois</h2>
                    <p>Howard Pyle</p>
                    <p>Publié en 1883</p>
                    <a href="détail-robin-des-bois.php" class="bouton">Détail</a>
                    <form method="post">
                        <button type="submit" 
                                name="book"
                                value="robin-des-bois"
                                class="bouton">
                            Ajouter au panier
                        </button>
                    </form>
                </div>
            </article>
            <article class="carte-livre">
                <img src="lesmisérables.jpg" alt="Les misérables">
                <div class="contenu-carte">
                    <h2>Les misérables</h2>
                    <p>Victor Hugo</p>
                    <p>Publié en 1862</p>
                    <a href="détail-les-misérables.php" class="bouton">Détail</a>
                    <form method="post">
                        <button type="submit" 
                                name="book"
                                value="les-misérables"
                                class="bouton">
                            Ajouter au panier
                        </button>
                    </form>
                </div>
            </article>
            <article class="carte-livre">
                <img src="lepetitchaperonrouge.jpg" alt="Le petit chaperon rouge">
                <div class="contenu-carte">
                    <h2>Le petit chaperon rouge</h2>
                    <p>Charles Perrault</p>
                    <p>Publié en 1697</p>
                    <a href="détail-le-petit-chaperon-rouge.php" class="bouton">Détail</a>
                    <form method="post">
                        <button type="submit" 
                                name="book"
                                value="le-petit-chaperon-rouge"
                                class="bouton">
                            Ajouter au panier
                        </button>
                    </form>
                </div>
            </article>
            <article class="carte-livre">
                <img src="lecomptedemonte-cristojpg.jpg" alt="Le compte de monte cristo">
                <div class="contenu-carte">
                    <h2>Le compte de monte cristo</h2>
                    <p>Alexander Dumas</p>
                    <p>Publié en 1844</p>
                    <a href="détail-le-compte-de-monte-cristo.php" class="bouton">Détail</a>
                    <form method="post">
                        <button type="submit" 
                                name="book"
                                value="le-compte-de-monte-cristo"
                                class="bouton">
                            Ajouter au panier
                        </button>
                    </form>
                </div>
            </article>
        </section>
        <section class="liste-livres">
            <h2>Liste des livres</h2>
            <article class="livre-horizontal">
                <img src="lepetitprince.jpg" alt="Le petit prince">
                <div class="info-livre">
                    <h3>Le petit prince</h3>
                    <p><strong>Auteur :</strong>Antoine de Saint-Exupéry</p>
                    <p><strong>Année :</strong>1943</p>
                    <a href="détail-le-petit-prince.php" class="bouton">Détail</a>
                    <form method="post">
                        <button type="submit" 
                                name="book"
                                value="le-petit-prince"
                                class="bouton">
                            Ajouter au panier
                        </button>
                    </form>
                </div>
            </article>
            <article class="livre-horizontal">
                <img src="harrypotter.jpg" alt="Harry Potter">
                <div class="info-livre">
                    <h3>Harry Potter</h3>
                    <p><strong>Auteur :</strong>J.K Rowling</p>
                    <p><strong>Année :</strong>1997</p>
                    <a href="détail-harry-potter.php" class="bouton">Détail</a>
                    <form method="post">
                        <button type="submit" 
                                name="book"
                                value="harry-potter"
                                class="bouton">
                            Ajouter au panier
                        </button>
                    </form>
                </div>
            </article>
            <article class="livre-horizontal">
                <img src="robindesbois.jpg" alt="Robin des bois">
                <div class="info-livre">
                    <h3>Robin des bois</h3>
                    <p><strong>Auteur :</strong>Howard Pyle</p>
                    <p><strong>Année :</strong>1883</p>
                    <a href="détail-robin-des-bois.php" class="bouton">Détail</a>
                    <form method="post">
                        <button type="submit" 
                                name="book"
                                value="robin-des-bois"
                                class="bouton">
                            Ajouter au panier
                        </button>
                    </form>
                </div>
            </article>
            <article class="livre-horizontal">
                <img src="lesmisérables.jpg" alt="Les misérables">
                <div class="info-livre">
                    <h3>Les misérables</h3>
                    <p><strong>Auteur :</strong>Victor Hugo</p>
                    <p><strong>Année :</strong>1862</p>
                    <a href="détail-les-misérables.php" class="bouton">Détail</a>
                    <form method="post">
                        <button type="submit"
                                name="book"
                                value="les-misérables"
                                class="bouton">
                            Ajouter au panier
                        </button>
                    </form>
                </div>
            </article>
            <article class="livre-horizontal">
                <img src="lepetitchaperonrouge.jpg" alt="Le petit chaperon rouge">
                <div class="info-livre">
                    <h3>Le petit chaperon rouge</h3>
                    <p><strong>Auteur :</strong>Charles Perrault</p>
                    <p><strong>Année :</strong>1697</p>
                    <a href="détail-le-petit-chaperon-rouge.php" class="bouton">Détail</a>
                    <form method="post">
                        <button type="submit"
                                name="book"
                                value="le-petit-chaperon-rouge"
                                class="bouton">
                            Ajouter au panier
                        </button>
                    </form>
                </div>
            </article>
            <article class="livre-horizontal">
                <img src="lecomptedemonte-cristojpg.jpg" alt="Le comte de monte-cristo">
                <div class="info-livre">
                    <h3>Le comte de Monte-Cristo</h3>
                    <p><strong>Auteur :</strong>Alexandre Dumas</p>
                    <p><strong>Année :</strong>1844</p>
                    <a href="détail-le-compte-de-monte-cristo.php" class="bouton">Détail</a>
                    <form method="post">
                        <button type="submit" 
                                name="book"
                                value="le-compte-de-monte-cristo"
                                class="bouton">
                            Ajouter au panier
                        </button>
                    </form>
                </div>
            </article>
        </section>
        <section class="apropos">
            <div>
                <h2>A propos de notre bibliothèque</h2>
                <p>Notre bibliothèque met en avant des oeuvres célèbres afin de permettre aux lecteurs de découvrir leurs auteurs, leurs dates de publication et leur histoire.</p>
            </div>
            <a href="nous-contacter.php" class="bouton">Nous contacter</a>
        </section>
        <section class="contact">
            <h2>Contact</h2>
            <div class="coordonnees">
                <p><strong>E-mail :</strong>bibliotheque@email.com</p>
                <p><strong>Téléphone :</strong>06 06 06 06 06</p>
            </div>
        </section>
    </main>
    <footer class="footer"><p>&copy; 2026 Ma bibliothèque</p></footer>
</body>
</html>
